...
**Track, compare, and compete on Claude Code, Codex CLI, Cursor, Kimi, Grok, GLM, Muse, and Pi token usage.**
...
Include local coding-agent usage from ccusage, Pi, Kimi Code, Grok CLI, GLM, Cursor, Muse Code, and other supported sources:
...
| `muse` | `~/.local/share/muse/sessions` (or `$XDG_DATA_HOME/muse/sessions`), main sessions and nested subagent transcripts | Zero (contributor-plan logs carry no pricing) |
